...
export default class ShoppingCart {
  ...
  async fetchCart() {
    try {
      ...
      //TODO: Cart tipine uygun veri gelmeyebilir
      this.cart = data;
      this.listProducts();
    } catch (networkError) {
  ...
  listProducts() {
    ...
    for (const product of this.cart.products) {
      ...
      const btnIncrease = this.ui.createButton("+", "btn", "inc" + id, () =>
        this.increaseQuantity(id),
      );
      const btnDecrease = this.ui.createButton("-", "btn", "dec" + id, () =>
      ...
      const btnClose = this.ui.createButton("X", "btn-close", "clo" + id, () =>
  ...
  increaseQuantity(id: number) {
    const a = this.cart.products.find((product) => product.id === id);
